...
(puerto **5432**, para no chocar con `turnosAlma`, que usa 5433).
...
1. **Base de datos**: en Coolify, crear un recurso **PostgreSQL** (puede ser el
   mismo servidor Postgres compartido con `turnosAlma`, con una base por
   proyecto) y crear la base `odontoturnos_db` con su usuario propio.
...
3. **Variables de entorno** (tab Environment Variables de la app): configurar
   ...
   - `DATABASE_URL`: connection string interno que muestra el recurso Postgres
     ...
     `postgresql://odontoturnos:PASSWORD@odontoturnos-db:5432/odontoturnos_db`).
